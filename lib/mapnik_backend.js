...
MapnikSource.prototype._normalizeURI = function(uri) {
    ...
    if (!uri.query.tileInfo) uri.query.tileInfo = 'web-mercator';
...
MapnikSource.prototype._createMetatileCache = function(options) {
    ...
    this._tileCache = new LockingCache(function(key) {
        ...
        var keys = source._renderMetatile({
            ...
            tileInfo: source._uri.query.tileInfo,
...
function convert(z, x, y, tileInfo) {
    switch (tileInfo) {
    case 'tms-global-mercator':
        return xyz.fromTMS([z, x, y]);
    case 'baidu':
        return xyz.fromBAIDU([z, x, y]);
    default:
        return [z, x, y];
    }
}
...
MapnikSource.prototype.getTile = function(z, x, y, callback) {
    ...
    var source = this;
    var tileInfo = source._uri.query.tileInfo;
    var c = convert(z, x, y, tileInfo);
    var wx = c[1], wy = c[2];

    var max = Math.pow(2, z);
    if (!isFinite(max) || wx >= max || wx < 0 || wy >= max || wy < 0) {
        return callback(new Error('Coordinates out of range: '+z+'/'+x+'/'+y));
    }

    var format = (this._info && this._info.format) || 'png';
    var key = [format, z, wx, wy].join(',');
